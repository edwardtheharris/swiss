...
class MultipartStream:
    def __init__(self, scope, path, look_for):
        ...
        if not hasattr(look_for, "decode"):
            raise TypeError("Expected bytes-like object for look_for.")
    ...
    def _find_boundary_end(
        self, newchunk, start_from=0, boundary_str=True, pre_crlf=0, post_crlf=0
    ):
        ...
        boundary = b"".join(
            [self._CRLF for i in range(pre_crlf)]
                ...
                for i in range(min(len(newchunk), len(boundary)) - 1, -1, -1):
                    if newchunk[i] == last_letter:
                        ...
                        if boundary.endswith(newchunk_portion.obj):
                            ...
                            if (
                                self._old_chunk.obj.obj + bytes(newchunk_portion)
                            ).endswith(boundary):
    ...
    def add_chunk(self, chunk):
        ...
        while True:
            if self._state == _States.READY:
                ...
                if start_idx >= 0:
                    self._cd_str += bytes(
                        mv_chunk[prev_start_idx : max(0, start_idx - boundary_size)]
                    )
                    ...
                    if start_idx < boundary_size:
                        self._cd_str = self._cd_str[: start_idx - boundary_size]
                    hdr_parse = self._parse_cd_str()
                    # be ready to open the file for writing if we encountered the correct form field
                    self._fh = (
                        open(
                            os.path.join(self._path, hdr_parse["file_name"].decode()),
                            "wb",
                        )
                        if hdr_parse["field_name"] == self._look_for
                        else None
                    )
                ...
                if self._fh is not None:
                    ...
                    if start_idx < 0:
                        ...
                        if self._saved_data_chunk is not None:
                            ...
                            if start_idx < boundary_size:
                                self._saved_data_chunk = self._saved_data_chunk[
                                    : start_idx - boundary_size
                                ]
                            self._fh.write(self._saved_data_chunk.obj)
                            self._saved_data_chunk = None
                        ...
                        if start_idx > boundary_size:
                            self._fh.write(
                                mv_chunk[prev_start_idx : start_idx - boundary_size].obj
                            )
                if start_idx >= 0:
                    if self._fh is not None:
                        self._fh.close()
                ...
                while start_idx < len(mv_chunk):
                    ...
                    start_idx += 1
                    if (
                        len(self._data_end_marker) == 2
                        and self._data_end_marker == b"\r\n"
                    ):
